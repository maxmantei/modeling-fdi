...
standata$priors <- 0
empty_stanmodel <- stan("models/gamma_poisson_hsy.stan", data = standata, iter = 0, chains = 0)
...
resid <- results_extended %>% 
  ...
  mutate(resid = FDI_AM - estimate,
         resid_pearson = resid/sqrt(estimate))
...
resid %>% ggplot(aes(x = log(estimate), y = resid_pearson)) +
  ...
  ylab(expression(hat(epsilon)/sqrt(hat(mu)))) +
  theme_minimal() +
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5))
resid %>% ggplot(aes(x = log(estimate), y = sqrt(abs(resid_pearson)))) +
  ...
  geom_smooth(color = "coral4") +
  xlab(expression(log(hat(mu)))) +
  ylab(expression(sqrt(abs(hat(epsilon)/sqrt(hat(mu)))))) +
  theme_minimal() +
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5))
