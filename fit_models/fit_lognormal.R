...
standata$priors <- 0
empty_stanmodel <- stan("models/lognormal_hsy.stan", data = standata, iter = 0, chains = 0)
...
# Correct calculation of Var(mu) ?
# Lognormal is NOT in exponential family...
# Correct interpretation of expectations?!
sigma_squared <- results$estimate[results$parameter == "sigma"]^2
...
resid <- results_extended %>% 
  ...
  mutate(resid = FDI_AM - exp(estimate + (0.5*sigma_squared)),
         resid_pearson = resid/((exp(sigma_squared-1)*exp(2*estimate+sigma_squared))),
         resid_log = log(FDI_AM) - estimate,
         resid_log_pearson = resid_log/results$estimate[results$parameter == "sigma"])

resid %>% ggplot(aes(x = FDI_AM, y = exp(estimate + (0.5*sigma_squared)))) + 
  ...
  xlab(expression(FDI)) +
  ylab(expression(exp(hat(mu)+0.5*hat(sigma)^2))) +
  theme_minimal() +
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5))
resid %>% ggplot(aes(x = log(FDI_AM), y = estimate)) + 
  geom_point(alpha = 0.8, color = "azure3") + 
  geom_abline(intercept = 0, slope = 1, color = "cornflowerblue", size = 1) +
  geom_smooth(color = "coral4") +
...
resid %>% ggplot(aes(x = estimate + (0.5*sigma_squared), y = resid_pearson)) +
  geom_point(alpha = 0.8, color = "azure3") +
  geom_smooth(color = "coral4") +
  ...
  theme_minimal() +
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5))
resid %>% ggplot(aes(x = estimate + (0.5*sigma_squared), y = sqrt(abs(resid_pearson)))) +
  geom_point(alpha = 0.8, color = "azure3") +
  geom_smooth(color = "coral4") +
  xlab(expression(hat(mu))) +
...
resid %>% ggplot(aes(x = estimate, y = resid_log_pearson)) +
  geom_point(alpha = 0.8, color = "azure3") +
  geom_smooth(color = "coral4") +
  coord_cartesian(ylim = c(-5, 5)) +
  xlab(expression(hat(mu))) +
  ...
  theme_minimal() +
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5))
resid %>% ggplot(aes(x = estimate, y = sqrt(abs(resid_log_pearson)))) +
  geom_point(alpha = 0.8, color = "azure3") +
  geom_smooth(color = "coral4") +
  xlab(expression(hat(mu))) +
  ylab(expression(sqrt(abs(hat(epsilon)/hat(sigma))))) +
...
standata$priors <- 0
empty_stanmodel <- stan("models/lognormal_hysyd.stan", data = standata, iter = 0, chains = 0)
